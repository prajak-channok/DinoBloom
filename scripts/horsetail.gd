...
func play_attack() -> void:
    ...
    # Temporary single-image attack presentation.
    # Replace AttackSprite with AnimatedSprite2D + SpriteFrames when animation art is available.
    get_tree().create_timer(0.08).timeout.connect(_finish_attack, CONNECT_ONE_SHOT)
...
func _combat_process(delta: float) -> void:
    ...
    var projectile_scene: PackedScene = load("res://scenes/plants/horsetail_projectile.tscn")
